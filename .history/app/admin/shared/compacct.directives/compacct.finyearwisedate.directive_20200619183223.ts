import { Directive, OnInit, ElementRef, Renderer2, Output, EventEmitter, Renderer, HostListener } from '@angular/core';
...
@Directive({
    ...
    selector: '[CompacctFinyearonly][checkFin]',
    providers: [NgModel]
})
export class CompacctFinyearwisedateDirective implements OnInit {
  ...
  @Output('check-fin') customSubmit: EventEmitter<any> = new EventEmitter();
  ...
  GetFinDetails() {
    ...
    if(fin[0].Fin_Year_Name) {
        ...
        if ((today > StartDate) && (today < EndDate)) {
          ...
          this.customSubmit.emit(endDate);
