...
export class DiplSupportTicketComponent implements OnInit {
  ...
  // CHANGE
  LoanCompanyChange() {
    this.BankTRNTypeChange();
    if(this.ObjLoanSearch.Loan_Company_Ledger) {
      const BankTrnType = this.LoanCompanyList.filter(item=> Number(item.Ledger_ID )=== Number(this.ObjLoanSearch.Loan_Company_Ledger));
      if(BankTrnType.length) {
        this.BankTrnTypeList = this.BankTrnTypeList.filter(item=> Number(item.Bank_Txn_Type_ID) === Number(BankTrnType[0].Bank_Txn_Type_ID));
        this.ObjLedger.Bank_Txn_Type = this.BankTrnTypeList[0].Txn_Type_Name;
        this.BankTRNTypeChange();
      }
