...
export class DiplSupportTicketComponent implements OnInit {
  ...
  CallSheetModalFlag = false;
  CallSheetDisplayModalFlag = false;
  ...
  clearData() {
    ...
    this.PreviousSuptTktList = [];
    this.CallSheetLlist =[];
    this.PartsList = [];
    this.ObjsupportTkt= new supportTkt();
    this.ObjviewcallSheet = new viewcallSheet();
    this.CallSheetModalFlag = false;
    this.CallSheetDisplayModalFlag = false;
  }
  onReject() {
  ...
  CallSheetModal (obj) {
    ...
    const end = this.ObjSearch.end_dt_time
      ...
      : this.DateService.dateConvert(new Date());
      const TempObj = {
        Sup_Ticket_ID: obj.Sup_Ticket_ID,
        Start_Date : start,
        End_Date: end
      }
      const obj2 = {
        "SP_String": "SUP_Call_Sheet",
        "Report_Name_String": "Browse_Support_Call_Sheet_With_Sup_Ticket_ID",
        "Json_Param_String": JSON.stringify([TempObj])
      }
      this.GlobalAPI.getData(obj2).subscribe((data:any)=>{
        this.CallSheetLlist = data.length ? data : [];
        if(this.CallSheetLlist.length){
          this.CallSheetModalFlag = true;
        }
        })
    }
  }
  ...
  viewcallSheet(col){
    ...
    if(col.Sup_Call_Sheet_ID) {
      ...
      this.GlobalAPI.getData(obj).subscribe((data:any)=>{
        ...
        this.CallSheetDisplayModalFlag = true;
      })
  ...
  // CHANGE
  GetPreviousSuptTktList () {
    this.PreviousSuptTktList = [];
    if (this.FootFallID || this.SubledgerID) {
      const tempObj = {
        Foot_Fall_ID : this.FootFallID ? this.FootFallID : '0',
        Sub_Ledger_ID : this.SubledgerID ? this.SubledgerID : '0'
      }
      const obj = {
        "Report_Name": "Get_Support_Ticket_Previous",
        "Json_Param_String" : JSON.stringify([tempObj])
      }
      this.GlobalAPI
          .CommonTaskData2(obj)
          .subscribe((data: any) => {
            this.PreviousSuptTktList = data.length ? data : [];
      });
    }
  }
  ...
  SearchSupportTicketList(valid) {
    ...
    this.SupportTktSearchFormSubmitted = true;
    if (valid) {
      ...
      const end = this.ObjSearch.end_dt_time
        ...
        const obj = {
          ...
          "Json_Param_String": JSON.stringify([{'Start_Date': start,'End_Date':end , 'Status_ID' : this.ObjSearch.Status_ID,'Assigned_To': this.ObjSearch.Asigned_To ? this.ObjSearch.Asigned_To : 0}])
        }
        this.GlobalAPI
            .CommonTaskData2(obj)
            .subscribe((data: any) => {
  ...
  GetDist() {
    ...
    if (DDistAssignTo.indexOf(item.Assigned_To) === -1) {
      DDistAssignTo.push(item.Assigned_To);
      this.DistAssignTo.push({label: item.Asigned_To_Name,value: item.Assigned_To});
  }
  ...
  FilterDist() {
    ...
    if (this.searchFields2.length) {
      let LeadArr = this.BackUpSupportTicketList.filter(function (e) {
        ...
        (DDistAssignTo.length ? DDistAssignTo.includes(e['Assigned_To']) : true)&&
  ...
  ShowCallSheets(obj) {
    if(obj.Followup_ID){
      const obj2 = {
        Followup_ID : window.btoa(obj.Followup_ID)
      }
      const navigationExtras: NavigationExtras = {
        queryParams: obj2,
      };
      this.router.navigate(['./DIPL_Support_Call_Sheet'], navigationExtras);
    }
  }
