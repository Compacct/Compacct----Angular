...
export class DIPLSupportCallSheetComponent implements OnInit {
  ...
  constructor(
    ...
    this.route.queryParams.subscribe(params => {
      // console.log(params);
      this.Param_Flag = window.btoa(params['Followup_ID']);
  ...
  ngOnInit() {
    ...
    this.Header.pushHeader({
     Header: "Support Call Sheet",
      Link: "Support Tkt -> Support Call Sheet"
    });
    this.GetAssigned();
    this.GetAllData();
    this.GetStatus();
  ...
  onReject(){}
 totalAmt(){
   console.log("this.serviceCharge",this.serviceCharge);
   console.log("this.otherCharge",this.otherCharge);
 ...
 GetAssigned(){
  ...
  this.GlobalAPI.getData(obj).subscribe((data:any)=>{
     ...
     for(let i = 0; i <= this.AssignedList.length ; i++){
       if(this.AssignedList[i].User_ID != this.$CompacctAPI.CompacctCookies.User_ID){
...
addReplaced(valid){
  ...
  if(valid){
    ...
    console.log("this.Replacedlist",this.Replacedlist);
  }
...
saveCallSheet(valid,Valid2){
  ...
  if(valid && Valid2 && this.Replacedlist.length){
    let saveObj = []
    this.ObjsupportTkt.Consuable_User_By = this.frnVal ? "Our Company" : this.ObjsupportTkt.Company_Name;
    console.log("this.ObjsupportTkt.Consuable_User_By",this.ObjsupportTkt.Consuable_User_By);
