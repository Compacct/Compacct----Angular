...
export class TutoLoamEmiTransferComponent implements OnInit {
  ...
  LoanCompanyChange() {
    ...
    if(this.ObjLoanSearch.Loan_Company_Ledger) {
      const BankTrnType = this.LoanCompanyList.filter(item=> item.Ledger_ID === this.ObjLoanSearch.Loan_Company_Ledger);
      if(BankTrnType.length) {
        this.BankTrnTypeList = this.BankTrnTypeList.filter(item=> item.Bank_Txn_Type_ID === BankTrnType[0].Bank_Txn_Type_ID);
        this.BankTRNTypeChange();
      }
