...
return flag;
  ...
  AddDetailList(valid) {
    ...
    if(valid && validFlag) {
      ...
      if(this.ObjJobData.Type_of_Work === 'Bridges and Culverts' && ItemFlag === 'HP') {
        ...
        for(let i = 0; i < this.DetailList2.length; i++) {
          for(let k = 0; k < this.DistinctStructure2.length; k++) {
            if (this.DetailList2[i].Structure_Details ===  this.DistinctStructure2[k].Sl_No ) {
              if(processed2.indexOf(this.DetailList2[i].Item_Code)<0) {
                processed2.push(this.DetailList2[i].Item_Code);
                this.DistinctStructure2[k].Sub_Item_Flag = true;
                this.DistinctStructure2[k].Sub_Item_List.push({
                  Item_Code:this.DetailList2[i].Item_Code,
                  Sl_No:this.DetailList2[i].Sl_No,
                  ToggleFlag: true,
                  Sub_Item_List : []
                });
                this.DistinctStructure2[k].Sub_Item_List.sort((a, b) => (Number(a.Sl_No) > Number(b.Sl_No)) ? 1 : -1);
              }
            }
          }
        }
        ...
        for(let i = 0; i < this.DetailList3.length; i++) {
          for(let k = 0; k < this.DistinctStructure3.length; k++) {
            if (this.DetailList3[i].Structure_Details ===  this.DistinctStructure3[k].Sl_No ) {
              if(processed2.indexOf(this.DetailList3[i].Item_Code)<0) {
                processed2.push(this.DetailList3[i].Item_Code);
                this.DistinctStructure3[k].Sub_Item_Flag = true;
                this.DistinctStructure3[k].Sub_Item_List.push({
                  Item_Code:this.DetailList3[i].Item_Code,
                  Sl_No:this.DetailList3[i].Sl_No,
                  ToggleFlag: true,
                  Sub_Item_List : []
                });
                this.DistinctStructure3[k].Sub_Item_List.sort((a, b) => (Number(a.Sl_No) > Number(b.Sl_No)) ? 1 : -1);
              }
            }
          }
        }
      ...
      this.DistinctItemCode3.sort((a, b) => (Number(a.Sl_No) > Number(b.Sl_No)) ? 1 : -1);
        ...
        if(this.ObjJobData.Type_of_Work === 'Bridges and Culverts'){
          ...
          for(let i = 0; i < this.DetailList.length; i++) {
            const struc = this.DetailList[i].structure_at_km +'/'+this.DetailList[i].structure_at_meter;
            if (processed3.indexOf(struc)<0) {
              processed3.push(struc);
              this.DistinctStructure.push({
                structure:struc+' Km. - '+ this.DetailList[i].Structured_Type,
          ...
          for(let i = 0; i < this.DetailList.length; i++) {
            for(let k = 0; k < this.DistinctStructure.length; k++) {
              if (this.DetailList[i].Structure_Details ===  this.DistinctStructure[k].Sl_No ) {
                if(processed2.indexOf(this.DetailList[i].Item_Code)<0) {
                  processed2.push(this.DetailList[i].Item_Code);
  ...
  GridValidationCheck() {
    ...
    if(this.ObjJobData.Chanage_From_KM || this.ObjJobData.Chanage_To_KM || this.ObjJobData.Chanage_From_M || this.ObjJobData.Chanage_To_M) {
      if(Chanage_From === Chanage_At ) {
        this.ChainageValidMgs = 'Both Chainage From '+Chanage_From+ 'and Chainage To '+Chanage_At+ 'are same';
        flag = false;
      }
    }
  ...
  GetItemConsumption(item,item_code , bridge?) {
    if(item.Loose_Qnty) {
      ...
      const totalLoseQnty = this.GetGridTotalLooseQnty(item ,bridge);
  ...
  GetTotalDia() {
    let count = 0;
    for(let i = 0; i < this.DetailList3.length; i++) {
      if(this.DetailList3[i].Total_Length_Rmt) {
        count += Number(this.DetailList3[i].Total_Length_Rmt);
      }
    }
    const valQnty =  typeof(count) === "number" ? count.toFixed(3) : Number(count).toFixed(3);
    return this.numberWithCommas(valQnty);
...
if(valid) {
  ...
  ShowStructureDetais(obj) {
    ...
    if(obj.Type_of_Work === 'Bridges and Culverts') {
