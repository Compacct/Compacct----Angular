...
return flag;
  ...
  GetAverageThickness(obj){
    ...
    const avgThickness =(Number(obj.Hight_Thikness.replace(/,/g, '')) /(Number(totalCompacctedQnty.replace(/,/g, '')) / ( Number(totalLooseQnty.replace(/,/g, '')))));
    return avgThickness.toFixed() + ' Cum.'
  }
