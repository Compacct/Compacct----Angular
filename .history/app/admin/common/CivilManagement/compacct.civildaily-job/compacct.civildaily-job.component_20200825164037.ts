        ...
        if(this.ObjJobData.Type_of_Work === 'Bridges and Culverts'){
          ...
          for(let i = 0; i < this.DistinctStructure.length; i++) {
            for(let k = 0; k < this.DistinctItemCode.length; k++) {
              if (this.DistinctStructure[i].structure ===  this.DistinctItemCode[k].structure ) {
                if(processed4.indexOf(this.DistinctItemCode[k].Item_Code)<0) {
                  processed4.push(this.DistinctItemCode[k].Item_Code);
                  this.DistinctStructure[k].Sub_Item_Flag = true;
                  this.DistinctStructure[k].Sub_Item_List.push({
                    DistinctItemCode:this.DistinctItemCode[k].Item_Code,
                    Sl_No:this.DistinctItemCode[k].Sl_No,
                    ToggleFlag2: true,
                  });
                  this.DistinctStructure[k].Sub_Item_List.sort((a, b) => (Number(a.Sl_No) > Number(b.Sl_No)) ? 1 : -1);
                }
              }
            }
          }
