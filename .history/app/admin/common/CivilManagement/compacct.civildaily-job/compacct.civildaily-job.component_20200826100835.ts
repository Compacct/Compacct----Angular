        ...
        if(this.ObjJobData.Type_of_Work === 'Bridges and Culverts'){
          ...
          for(let i = 0; i < this.DistinctStructure.length; i++) {
            for(let k = 0; k < this.DistinctStructure[i].Sub_Item_List.length; k++) {
              const StructureDetails =  this.DistinctStructure[i].Sub_Item_List.length[k].Item_Code;
              const ItemCode =  this.DistinctStructure[i].Sub_Item_List.length[k].Item_Code;
              ...
              if(tempArr1.length) {
                ...
                for(let p = 0; p < tempArr1.length; p++) {
                  if (tempArr1[p].bridge_item ) {
                    if(processed2.indexOf(tempArr1[i].bridge_item)<0) {
                      processed2.push(tempArr1[i].bridge_item);
                      this.DistinctStructure[i].Sub_Item_List.length[k].Sub_Item_Flag = true;
                      this.DistinctStructure[i].Sub_Item_List.length[k].Sub_Item_List.push({
                        bridge_item:tempArr1[i].bridge_item,
                        Sl_No:tempArr1[i].bridge_item_Sl_No,
                        ToggleFlag2: true,
                      });
                      // this.DistinctItemCode3[k].Sub_Item_List.sort((a, b) => (Number(a.Sl_No) > Number(b.Sl_No)) ? 1 : -1);
                    }
