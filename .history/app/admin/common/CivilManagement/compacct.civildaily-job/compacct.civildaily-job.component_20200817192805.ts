...
return flag;
  ...
  GetBridgeTotalQtny() {
    ...
    if((this.ObjJobData.Bridge_Item_Height  && this.ObjJobData.Bridge_Item_Breath ) && (this.ObjJobData.bridge_item_length  && this.ObjJobData.bridge_item)){
      ...
      this.ObjJobData.Bridge_Item_Total_Qty = FlterVal;
    }
  }
  GetQnty() {
  ...
  calculateHpLength(){
    ...
    if(this.ObjJobData.Length_per_piece && this.ObjJobData.Number) {
      const val =  Number(this.ObjJobData.Number) * Number(this.ObjJobData.Length_per_piece);
  ...
  GetTotalQnt(obj) {
    ...
    return this.numberWithCommas(qunty);
  }
  GetSubTotalQnt(obj) {
  ...
  GetItemConsumption(item,item_code) {
    if(item.Loose_Qnty) {
      ...
      const n = (Number(totalcompact.replace(/,/g, '')) / ( Number(totalLoseQnty.replace(/,/g, ''))));
      return n.toFixed(2) +' %';
    }else {
      return '-';
    }
  ...
  if(valid) {
      ...
      const UrlAddress = "/BL_Txn_Civil_Daily_Job/Create_Structured_Type";
      const obj = {
        Structured_Type : this.PartyName
       };
      this.$http.post(UrlAddress, obj).subscribe((data: any) => {
        console.log(data)
      if (data.success) {
  ...
  if(valid) {
      ...
      const UrlAddress = "/BL_Txn_Civil_Daily_Job/Create_Structured_Type";
      const obj = {
        Structured_Type : this.ReceivedFromName
       };
      this.$http.post(UrlAddress, obj).subscribe((data: any) => {
        console.log(data)
      if (data.success) {
