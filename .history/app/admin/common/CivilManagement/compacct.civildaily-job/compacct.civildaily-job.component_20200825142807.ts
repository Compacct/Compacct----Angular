...
return flag;
  ...
  GetDistinctItemCode(obj,toggle?,obj1?) {
    const processed = [] , processed2 = [];
    let DistinctItemCode = [];
    for(let i = 0; i < this.DetailList.length; i++) {
      if (this.DetailList[i].Structure_Details ===   obj.Sl_No ) {
        if (processed.indexOf(this.DetailList[i].Item_Code)<0) {
          processed.push(this.DetailList[i].Item_Code);
          DistinctItemCode.push({
            Item_Code:this.DetailList[i].Item_Code,
            Sl_No:this.DetailList[i].Sl_No,
            ToggleFlag: true,
            Sub_Item_Flag : false,
    ...
    for(let i = 0; i < this.DetailList.length; i++) {
      for(let k = 0; k < DistinctItemCode.length; k++) {
        if (this.DetailList[i].Item_Code ===  DistinctItemCode[k].Item_Code && this.DetailList[i].bridge_item ) {
          if(processed2.indexOf(this.DetailList[i].bridge_item)<0) {
            processed2.push(this.DetailList[i].bridge_item);
            if (toggle) {
              if (this.DetailList[i].Grid_Index ===  obj1.Grid_Index) {
                DistinctItemCode[k].Sub_Item_Flag = false;
              }
            } else {
              DistinctItemCode[k].Sub_Item_Flag = true;
            }
            DistinctItemCode[k].Sub_Item_List.push({
              bridge_item:this.DetailList[i].bridge_item,
              Sl_No:this.DetailList[i].bridge_item_Sl_No,
              ToggleFlag2: true,
            });
    ...
    console.log(DistinctItemCode);
    return DistinctItemCode;
  }
