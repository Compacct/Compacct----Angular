...
ChainageValidMgs = '';
  ...
  GetStructureTypeDetail() {
    this.$http
    .get("/BL_Txn_Civil_Daily_Job/Get_Structured_Details_Json")
      .subscribe((data: any) => {
        this.StructureTypeDetailList = data ? JSON.parse(data) : [];
  ...
  ChecItemCode ():string {
    if(this.ObjJobData.Item_Code) {
      let flag;
      switch(this.ObjJobData.Item_Code) {
        case 'HYSD BAR':
          flag = 'BAR';
          break;
        case 'HP':
...
return flag;
  ...
  ChangeStructureDetails(){
    ...
    if(this.ObjJobData.Structure_Details) {
      const obj = $.grep(this.StructureTypeDetailList,function(obj){ return obj.val === this.ObjJobData.Structure_Details})[0];
  ...
  GetBridgeTotalQtny() {
    this.ObjJobData.QNTY =  undefined;
    if((this.ObjJobData.Bridge_Item_Height  && this.ObjJobData.Bridge_Item_Breath ) && (this.ObjJobData.bridge_item_length  && this.ObjJobData.bridge_item)){
  ...
  AddTrip() {
    ...
    if(this.ObjJobData.Trip_By && this.ObjJobData.No_Of_Trip) {
  ...
  GetTotalQnt(obj) {
    ...
    if(this.ObjJobData.Type_of_Work === 'Bridges and Culverts') {
      for(let i = 0; i < arrTemp.length; i++) {
        if(arrTemp[i].QNTY) {
          qunty += Number(arrTemp[i].Bridge_Item_Total_Qty);
        }
      }
    }
    if(this.ObjJobData.Type_of_Work === 'Road Works') {
  ...
  AddDetailList(valid) {
    ...
    if(valid && validFlag) {
      ...
      this.tempArr.forEach(function(obj,ind) {
        if(this.tempArr[ind].Item_Code === this.ObjJobData.Item_Code) {
          this.tempArr[ind].Trip_Arr = this.TripTempArr;
        }
      });
  ...
  getTripByItemCode(item) {
    const arrTemp = $.grep(this.DetailList,function(val){return val.Item_Code === item})[0];
    let tempArr = arrTemp.Trip_Arr.length ? arrTemp.Trip_Arr : [];
    // for(let i = 0; i < arrTemp.length; i++) {
    //   if(arrTemp[i].Trip_Arr.length) {
    //     for(let k = 0; k < arrTemp[i].Trip_Arr.length; k++) {
    //       tempArr.push(arrTemp[i].Trip_Arr[k]);
  ...
  viewTripTempArrFromDetails (obj) {
    this.TripTempArrFromDetails = []
      const arrTemp = $.grep(this.DetailList,function(val){return val.Item_Code === obj.Item_Code})[0];
      let tempArr =  arrTemp.Trip_Arr.length ? arrTemp.Trip_Arr : [];;
      // for(let i = 0; i < arrTemp.length; i++) {
      //   if(arrTemp[i].Trip_Arr.length) {
      //     for(let k = 0; k < arrTemp[i].Trip_Arr.length; k++) {
