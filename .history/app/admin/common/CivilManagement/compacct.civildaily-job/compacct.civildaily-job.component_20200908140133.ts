  ...
  checkValidChainage(){
    ...
    if((this.ObjJobData.Chanage_From_M || this.ObjJobData.Chanage_From_KM) && (this.ObjJobData.Chanage_To_KM || this.ObjJobData.Chanage_To_M)) {
      ...
      if((this.ObjJobData.Chainage_From_km || this.ObjJobData.Chainage_From_meter) && (this.ObjJobData.Chainage_To_km || this.ObjJobData.Chainage_To_meter)) {
        ...
        const Chanage_From1 = Number(this.ObjJobData.Chanage_From_KM) + '.' + this.ObjJobData.Chanage_From_M;
        const  Chanage_To2 = Number(this.ObjJobData.Chanage_To_KM) + '.' + this.ObjJobData.Chanage_To_M;
        const checkFrom = this.isBetween(Number(Chanage_From),Number(Chanage_From1),Number(Chanage_To2),'Tovalid');
        const checkTo = this.isBetween(Number(Chanage_To),Number(Chanage_From1),Number(Chanage_To2),'Fromvalid');
        if(checkFrom || checkTo) {
          console.log('found between');
          const Tostr =  'Chainage To : ' + Chanage_To2 + ' of Side ' + this.ObjJobData.Side + ' Already Exits.';
          const Fromstr =  'Chainage From : '+ Chanage_From1 + ' of Side ' + this.ObjJobData.Side + ' Already Exits.';
          const Allstr =  'Chainage From : '+ Chanage_From1 + ' | Chainage To : ' + Chanage_To2 +' of Side ' + this.ObjJobData.Side + ' Already Exits.';
          this.ChainageValidMgs = checkFrom && checkTo ? Allstr : checkFrom ? Fromstr : checkTo ? Tostr: '';
          flag = false;
        } else {
          const checkMax = Number(Chanage_From) < Number(Chanage_From1);
          const checkMaxTo = checkMax ? Number(Chanage_To) > Number(Chanage_From1) : false;
          if(checkMaxTo) {
            console.log('found between')
            const Tostr =  'Chainage To : ' + Chanage_To2 + ' of Side ' + this.ObjJobData.Side + ' Already Exits.';
          const Fromstr =  'Chainage From : '+ Chanage_From1 + ' of Side ' + this.ObjJobData.Side + ' Already Exits.';
