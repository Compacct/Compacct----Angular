...
waitLoader = false;
  ...
  GetStructureTypeDetail2() {
    this.$http
    .get("/BL_Txn_Civil_Daily_Job/Get_Structured_Details_Concrete")
      .subscribe((data: any) => {
    ...
    if(this.StructureTypeDetailList2.length){
      ...
      for(let i = 0; i < this.StructureTypeDetailList2.length; i++) {
        if(this.StructureTypeDetailList2[i].Structure_Side === this.ObjJobData.Structure_Side){
          const checkFrom = this.isBetween(Number(Chanage_From),Number(this.StructureTypeDetailList2[i].Chainage_From_km),Number(this.StructureTypeDetailList2[i].Chainage_From_meter),'Tovalid');
          const checkTo = this.isBetween(Number(Chanage_To),Number(this.StructureTypeDetailList2[i].Chainage_To_km),Number(this.StructureTypeDetailList2[i].Chainage_To_meter),'Fromvalid');
          if(checkFrom || checkTo) {
            console.log('found between');
            const Tostr =  'Chainage To : ' + this.StructureTypeDetailList2[i].Chanage_To + ' of Side ' + this.ObjJobData.Structure_Side + ' Already Exits.';
            const Fromstr =  'Chainage From : '+ this.StructureTypeDetailList2[i].Chanage_From + ' of Side ' + this.ObjJobData.Structure_Side + ' Already Exits.';
            const Allstr =  'Chainage From : '+ this.StructureTypeDetailList2[i].Chanage_From + ' | Chainage To : ' + this.StructureTypeDetailList2[i].Chanage_To +' of Side ' + this.ObjJobData.Side + ' Already Exits.';
            this.ChainageValidMgs = checkFrom && checkTo ? Allstr : checkFrom ? Fromstr : checkTo ? Tostr: '';
            flag = false;
              break;
          } else {
            const checkMax = Number(Chanage_From) < Number(this.StructureTypeDetailList2[i].Chanage_From);
            const checkMaxTo = checkMax ? Number(Chanage_To) > Number(this.StructureTypeDetailList2[i].Chanage_From) : false;
            if(checkMaxTo) {
              console.log('found between')
              const Tostr =  'Chainage To : ' + this.StructureTypeDetailList2[i].Chanage_To + ' of Side ' + this.ObjJobData.Structure_Side + ' Already Exits.';
            const Fromstr =  'Chainage From : '+ this.StructureTypeDetailList2[i].Chanage_From + ' of Side ' + this.ObjJobData.Structure_Side + ' Already Exits.';
