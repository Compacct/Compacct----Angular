...
export class CompacctRunningBillComponent implements OnInit {
  ...
  GetPreviousRunningBill() {
    ...
    this.$http
    .get("/BL_Txn_Civil_Running_Bill_Details/Get_Running_Bill_Details?Running_Bill_Sl_No="+ this.ObjRunningBill.Running_Bill_Sl_No_ID +"&Project_ID="+this.ObjRunningBill.Project_ID)
      .subscribe((data: any) => {
        console.log(data);
  ...
  calculateAmount() {
    ...
    if(this.ObjRunningBill.Qnty_Executed_Upto_Date && this.ObjRunningBill.Estimated_Rate) {
      ...
      if(this.ObjRunningBill.Qnty_Executed_Upto_Date && !this.PreviousRABill.length) {
        ...
        if(this.PreviousRABill.length) {
          const val = (Number(this.ObjRunningBill.Qnty_Executed_Upto_Date) - Number(this.PreviousRABill[0].Qnty_Executed_Of_This_Bill)).toFixed(3);
