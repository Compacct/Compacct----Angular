...
export class CompacctRunningBillComponent implements OnInit {
  ...
  mergeObj () {
    ...
    for(let i = 0; i < this.RunningDetailList.length;i++) {
      for(const key in this.ObjRunningBill) {
        if(this.RunningDetailList[i][key] === undefined || this.RunningDetailList[i][key] === null || this.RunningDetailList[i][key] === NaN){
          this.RunningDetailList[i][key] = this.ObjRunningBill[key];
        }

      }
    }
    console.log(this.RunningDetailList)
  }
