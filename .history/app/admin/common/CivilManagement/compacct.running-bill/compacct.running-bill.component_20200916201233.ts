...
export class CompacctRunningBillComponent implements OnInit {
  ...
  GetPreviousRunningBill() {
    ...
    this.$http
    .get("/BL_Txn_Civil_Running_Bill_Details/Get_Previous_Running_Bill_Details?Running_Bill_Sl_No_ID="+ this.ObjRunningBill.Running_Bill_Sl_No_ID +"&Project_ID="+this.ObjRunningBill.Project_ID)
      .subscribe((data: any) => {
  ...
  AddRunDetailList(valid) {
    ...
    if(valid) {
      ...
      this.RunningDetailList.sort((a, b) => (Number(a.Item_Sl_No) > Number(b.Item_Sl_No)) ? 1 : -1);
  ...
  GetAllCalculation() {
    ...
    if(this.PreviousRABill.length) {
      for(let i = 0; i < this.PreviousRABill.length;i++) {
        Upto_Date_Amount += Number(this.PreviousRABill[i].Upto_Date_Amount);
      }
    }
    ...
    const PercentageVal_upto = (( Number(percentage_upto) / 100) * Number(Upto_Date_Amount));
    const Rate_upto = n_upto ?  (Number(Upto_Date_Amount) - PercentageVal_upto).toFixed() :  (Number(Upto_Date_Amount) + PercentageVal_upto).toFixed();

    this.ObjRunningBill.Total_This_Bill_Amount_upto = Upto_Date_Amount.toFixed(2);
    ...
    if(this.PreviousRABill.length) {
      this.ObjRunningBill.Deduct_Work_Value_upto = this.PreviousRABill[0].Grand_Total_Amount_This_Bill;
    }
    const Net_Value_upto = (Number(this.ObjRunningBill.Grand_Total_Amount_This_Bill_upto) - Number(this.ObjRunningBill.Deduct_Work_Value_upto) ).toFixed();
    ...
    const PercentageVal  = (( Number(percentage ) / 100) * Number(This_Bill_Amount));
    const Rate  = n  ?  (Number(This_Bill_Amount) - PercentageVal ).toFixed() :  (Number(This_Bill_Amount) + PercentageVal ).toFixed();

    this.ObjRunningBill.Total_This_Bill_Amount = This_Bill_Amount.toFixed(2);
    ...
    this.ObjRunningBill.Grand_Total_Amount_This_Bill  = (Number(this.ObjRunningBill.Total_Gross_Amount_This_Bill ) + Number(this.ObjRunningBill.GST_Amount_This_Bill )).toFixed(2);
     ...
     this.ObjRunningBill.Deduct_Work_Value = 0;
    const Net_Value  = (Number(this.ObjRunningBill.Grand_Total_Amount_This_Bill ) - Number(this.ObjRunningBill.Deduct_Work_Value )).toFixed();
    this.ObjRunningBill.Net_Value  = Number(Net_Value ).toFixed(2);
  }
  mergeObj () {
    let TempArray = [];
    const obj ={
      Total_This_Bill_Amount : this.ObjRunningBill.Total_This_Bill_Amount,
      Quoted_Percentage_Amount_This_Bill : this.ObjRunningBill.Quoted_Percentage_Amount_This_Bill,
      Total_Gross_Amount_This_Bill : this.ObjRunningBill.Total_Gross_Amount_This_Bill,
      GST_Percentage : this.ObjRunningBill.GST_Percentage,
      GST_Amount_This_Bill : this.ObjRunningBill.GST_Amount_This_Bill,
      Grand_Total_Amount_This_Bill : this.ObjRunningBill.Grand_Total_Amount_This_Bill,
      Deduct_Work_Value : this.ObjRunningBill.Deduct_Work_Value,
      Net_Value : this.ObjRunningBill.Net_Value,
    }
    for(let i = 0; i < this.RunningDetailList.length;i++) {
      const temObj = {...this.RunningDetailList[i],...obj};
  ...
  SaveRABill() {
      if (this.RunningDetailList.length) {
        ...
        const obj = { Civil_Running_Bill_String: this.mergeObj()};
