...
export class CompacctRunningBillComponent implements OnInit {
  ...
  GetAllCalculation() {
    ...
    for(let i = 0; i < this.RunningDetailList.length;i++) {
      Upto_Date_Amount += this.RunningDetailList[i].Upto_Date_Amount;
    ...
    this.ObjRunningBill.Total_Gross_Amount_This_Bill = Rate.toFixed();
    this.ObjRunningBill.Quoted_Percentage_Amount_This_Bill = PercentageVal.toFixed();
