...
// }
getTry (data) {
  const DistinctCity =  Array.from([...new Set(data.map(item => item.Intro_Member_ID))]);
  console.log(DistinctCity)
  DistinctCity.forEach(item =>{
    const filterMemberID = data.filter(element => (element.Member_ID === DistinctCity));
