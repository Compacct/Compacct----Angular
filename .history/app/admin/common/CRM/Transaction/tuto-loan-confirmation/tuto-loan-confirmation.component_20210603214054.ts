...
//       if(Arr.length &&  Arr.filter(element2 => element.Member_Name === element2.label).length) {
...
//           const filterMemberID2 = data.filter(item => (element1.Member_ID === item.Intro_Member_ID));
//           if(filterMemberID2.length){
//             Obj.children.push({label : element1.Member_Name})
//             Obj.children.forEach(element3 => {
//               element3.children = [];
//               element3.children.push({label : element3.Member_Name})
//             })
//           }
//         });
//       }
...
getTry (data) {
  ...
  let DistinctIntro = [];
  data.forEach((item) => {
    if (Distinct.indexOf(item.Intro_Member_ID) === -1) {
      Distinct.push(item.Intro_Member_ID);
      DistinctIntro.push({Intro_Member_ID : item.Intro_Member_ID, Member_ID : item.Member_ID}) ;
    }
  })
  console.log(DistinctIntro)
  DistinctIntro.forEach(item =>{
    const filterMemberID = data.filter(element => (element.Intro_Member_ID === item.Member_ID));
    console.log(filterMemberID)
  })
}
