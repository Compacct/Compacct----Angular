...
TaskCustomerRadioFlag = 'NewCustomer';
  ...
  SaveAddress(valid) {
    ...
    if (valid ) {
      ...
      this.$http
        .post("/BL_CRM_Lead_Management_V2/Create_Address", { _BL_CRM_Txn_Enq_Address_details: this.ObjAddress })
        .subscribe((data: any) => {
          if (data.success === true) {
              this.compacctToast.clear();
              this.compacctToast.add({
  ...
  SaveTask(valid) {
    ...
    if (valid ) {
      ...
      // this.ObjLead.Status_ID = this.ObjLead.Foot_Fall_ID ? 2:1;
      this.$http
        .post("/BL_CRM_Txn_Enq_Task/Insert_Enq_Task", { BL_CRM_Txn_Enq_Task: JSON.stringify([this.ObjTask]) })
        .subscribe((data: any) => {
          if (data.success === true) {
              this.compacctToast.clear();
              this.compacctToast.add({
