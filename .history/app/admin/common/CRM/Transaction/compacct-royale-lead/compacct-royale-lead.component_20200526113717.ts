...
TaskCustomerRadioFlag = 'NewCustomer';
  ...
  GetAddressList (footfallid) {
    ...
    if(footfallid){
      ...
      this.$http
        ...
        .subscribe((data: any) => {
          console.log(data)
          this.AddressListView = data ? JSON.parse(data) : [];
          this.CreateLeadModal = true;
        });
    }
  }
  ...
  SaveAddress(valid) {
    ...
    if (valid ) {
      ...
      this.$http
        .post("/BL_CRM_Lead_Management_V2/Create_Address", { _BL_CRM_Txn_Enq_Address_details: JSON.stringify([this.ObjAddress]) })
        .subscribe((data: any) => {
          if (data.success === true) {
              this.compacctToast.clear();
              this.compacctToast.add({
  ...
  SaveStatus(valid) {
    ...
    if (valid ) {
      this.Spinner = true;
      const obj = new HttpParams().set("ID", this.ObjStaus.Foot_Fall_ID )
  ...
  SaveTask(valid) {
    ...
    if (valid ) {
      ...
      // this.ObjLead.Status_ID = this.ObjLead.Foot_Fall_ID ? 2:1;
      this.$http
        .post("/BL_CRM_Lead_Management_V2/Create_Address", { _BL_CRM_Txn_Enq_Address_details: JSON.stringify([this.ObjTask]) })
        .subscribe((data: any) => {
          if (data.success === true) {
              this.compacctToast.clear();
              this.compacctToast.add({
