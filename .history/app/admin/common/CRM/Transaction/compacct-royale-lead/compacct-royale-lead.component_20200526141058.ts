...
TaskCustomerRadioFlag = 'NewCustomer';
  ...
  CreateLead(){
    ...
    this.Spinner = false;
    this.CreateLeadModalHeader = 'Create Lead';
  }
  getStatus(id){
  ...
  SaveStatus(valid) {
    ...
    if (valid ) {
      this.Spinner = true;
      const params = new HttpParams().set("Foot_Fall_ID", this.ObjStaus.Foot_Fall_ID )
      .set("Status_ID", this.ObjStaus.Status_ID );
      this.$http
        .post("/BL_CRM_Lead_Management_V2/Update_Lead_Status?Foot_Fall_ID="+this.ObjStaus.Foot_Fall_ID+"&Status_ID="+this.ObjStaus.Status_ID,{})
        .subscribe((data: any) => {
          if (data.success === true) {
              this.compacctToast.clear();
              this.compacctToast.add({
