...
export class TutoPendsubscriptionComponent implements OnInit {
  ...
  MakePayement(obj) {
    const Link = '/Tutopia_Order_Payment?Subscription_Txn_ID='+ window.btoa(obj.Subscription_Txn_ID)+'&Menu_Ref_ID='+window.btoa(this.$CompacctAPI.CompacctCookies.Menu_Ref_ID)+'&Foot_Fall_ID='+window.btoa(obj.Foot_Fall_ID);
    window.open(Link);
  }
  PrintOrderAspx(obj){
    if(obj.Order_No) {
