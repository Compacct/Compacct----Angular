...
export class TutoPendsubscriptionComponent implements OnInit {
  ...
  ngOnInit() {
    this.items = ["CONFIRMED", "PENDING", "A/C NOT CONF"];
  ...
  Billcreation(col) {
    ...
    const Link = '/Tutopia_Student_Order/Index?Subscription_Txn_ID='+ window.btoa(col.Subscription_Txn_ID) +'&Menu_Reff_ID='+ window.btoa(this.$CompacctAPI.CompacctCookies.Menu_Ref_ID);
     window.open(Link);
  }
// EXPORT TO EXCEL
