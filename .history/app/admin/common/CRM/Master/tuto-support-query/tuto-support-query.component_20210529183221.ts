...
export class TutoSupportQueryComponent implements OnInit {
  ...
  BrowseTikStatusChanged() {
    if(this.ObjSearchTicket.Status) {
      ...
      if(this.$CompacctAPI.CompacctCookies.User_Type === 'U') {
        this.DefaultEnableChanger = this.ObjSearchTicket.Status == '2' ? '' : 'true';
      }
      if(this.$CompacctAPI.CompacctCookies.User_Type === 'A') {
  ...
  GetDistinct() {
    ...
    this.SupportQuerylist.forEach((item) => {
      ...
      if (DStatus.indexOf(item.Asigned_To) === -1) {
        DStatus.push(item.Asigned_To);
        this.DistStatus.push({ label: item.Asigned_To_Name, value: item.Asigned_To });
      }
