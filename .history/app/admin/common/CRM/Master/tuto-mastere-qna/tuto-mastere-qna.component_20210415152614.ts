...
export class TutoMastereQNAComponent implements OnInit {
  ...
  if(valid){
    ...
    const newObj = {
      ...
      Reply_Details:this.StringToHex(this.ObjAnswer.Reply_Details),
      is_public: this.ObjAnswer.is_public
    }
    ...
    const obj = {
      "Json_Param_String" : JSON.stringify([EditJSon])
    }
    this.$http.post('/Common/Create_Common_task?Report_Name=Update_Answer_Support',obj,httpOptions).subscribe((data:any)=>{
      //console.log(data);
    ...
    this.GlobalAPI.CommonTaskData(obj).subscribe((data:any)=>{
      if(data[0].Column1 === 'success') {
        ...
        this.compacctToast.add({
          key: "compacct-toast",
          severity: "success",
          summary: "Question ID : " + QuesObj.Tutorial_Question_ID,
