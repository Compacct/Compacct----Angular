...
add(valid) {
  ...
  var productObj = {
    ...
    // Flavour : this.ObjaddbillForm.Flavour,
    ...
    Sale_rate : Number(this.ObjaddbillForm.Sale_rate),
  ...
  console.log("flavour",this.ObjaddbillForm.Flavour)
  ...
  if(this.productSubmit.length) {
    ...
    this.productSubmit.forEach(item => {
      const obj = {
          ...
          Order_Taken_By : item.Order_Order_Taken_By,
          Rate : item.Sale_rate,
          Qty : item.Stock_Qty,
          Amount : item.Amount,
...
Edit(eROW){
  //this.DocNO = undefined;
    console.log("masterProduct",eROW);
    this.clearData();
