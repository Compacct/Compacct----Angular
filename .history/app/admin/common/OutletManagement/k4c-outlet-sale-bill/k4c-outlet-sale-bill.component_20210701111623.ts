...
export class K4cOutletSaleBillComponent implements OnInit {
  ...
  GetProductTypeFilterList(){
    this.ProductTypeFilterSelected = []
    const tempobj = {
    ...
    const obj = {
     ...
     this.GlobalAPI.getData(obj).subscribe((data:any)=>{
       ...
       // this.ProductTypeFilterSelected = data;
  ...
  chipclick(e) {
    ...
    if(e.value.length) {
      let LeadArr = this.selectitem.filter(function (e) {
        return (this.ProductTypeFilterSelected.length ? this.ProductTypeFilterSelected.includes(e['Product_Type_ID']) : true)
      });
    ...
    // if(e.text){
    //   if(e.selected) {
    //     const temp =  this.selectitem.filter(item=> item.Product_Type === e.text);
    //     this.ProductTypeFilterSelected.push(temp[0]);
    //   } else {
    //     const temp =  this.ProductTypeFilterSelected.filter(item=> item.Product_Type === e.text);
    //     this.ProductTypeFilterSelected.push(temp[0]);
