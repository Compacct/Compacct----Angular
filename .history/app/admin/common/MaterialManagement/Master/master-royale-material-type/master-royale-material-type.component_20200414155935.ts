...
export class MasterRoyaleMaterialTypeComponent implements OnInit {
  ...
  ngOnInit() {
    this.Header.pushHeader({
      ...
      Link: " Material Management -> Master -> Material Type"
    });
...
// CHANGE
AddMaterialType(valid){
  this.NewMaterialFormSubmitted = true;
  if(valid){
    this.MaterialTypOnlyLists.push({
      label: this.NewMaterialType,
      value: this.NewMaterialType
    });
    this.NewMaterialFormSubmitted = false;
    this.NewMaterialType = undefined;
  }
}

EditMaterialType(obj){
  if(obj.Master_Product_Material_Type_ID){
    this.ClearData();
