...
@Component({
  selector: 'app-outlet-stock-transfer-auto-batch',
  templateUrl: './outlet-stock-transfer-auto-batch.component.html',
  styleUrls: ['./outlet-stock-transfer-auto-batch.component.css'],
  providers: [MessageService],
  encapsulation: ViewEncapsulation.None
})
export class OutletStockTransferAutoBatchComponent implements OnInit {
  items = [];
  tabIndexToView = 0;
  menuList = [];
  buttonname = "Create";
...
if(valid && this.GetSelectedBatchqty()){
  ...
  var qty = this.Objadditem.Issue_Qty;
  var count = 0; // to count how many to shift 
  var finalBatch;
  this.BatchList.forEach(element => {

  if(element.Batch_NO === this.Objadditem.Batch_No){
    batch_show = element.Batch_No_Show;
    batch_id = element.Batch_NO;
    batch_qty = element.Qty
    }
...
for(var i =0 ; i<count; i++) {
 var batchno = this.BatchList.shift();
...
GetSelectedBatchqty() {
  ...
  const sameproductwithbatch = this.productDetails.filter(item=> Number(item.Product_ID) === Number(this.Objadditem.Product_ID) );
  ...
  var TotalBatch = undefined;
   ...
   const baychqtyarr = this.BatchList;
...
delete(id){
  let temarr = [];
   for(let i = 0; i < this.productDetails.length; i++){
     if (this.productDetails[i].Product_ID.toString() !== id.toString()) {
       temarr.push(this.productDetails[i]);
     }
 }
 this.productDetails = [...temarr];
