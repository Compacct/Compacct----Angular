...
export class NepalPurchaseRequestNegotiatePriceComponent implements OnInit {
  ...
  this.PaymentTramsSelect = undefined;
  ...
  getPaymentMethod() {
    const tempobj = {
      Purchase_Type_ID: this.PurchaseTypeSelect 
      }
    const obj = {
      "SP_String": "sp_Bl_Txn_Purchase_Request",
      "Report_Name_String": "Get_Data_Payment_Terms",
  ...
  getDeliveryMethod() {
    ...
    const obj = {
      "SP_String": "sp_Bl_Txn_Purchase_Request",
      "Report_Name_String": "Get_Data_Delivery_Terms",
      "Json_Param_String": JSON.stringify([tempobj])
    }
    this.GlobalAPI.getData(obj).subscribe((data: any) => {
      if(data.length) {
       ...
       this.DeliveryTermsList = data;
      }
       else {
        this.DeliveryTermsList = [];
  
      }
      //console.log("DeliveryTermsList",this.DeliveryTermsList)
    })
  }
  GetCurrency(){
    const obj = {
  ...
  negotiateSave(valid:any) {
    this.vendorSelectionFormSubmit = true
    if(valid && this.ObjnegotiatePrice.Purchase_Request_No){
      let saveData:any=[]
       this.prList.forEach((ele:any) => {
        saveData.push({
          Purchase_Request_No: this.ObjnegotiatePrice.Purchase_Request_No,
          Purchase_Type_ID: this.PurchaseTypeSelect,
          Payment_Term_ID: this.PaymentMethodSelect,
          Payment_Term_Details : this.PaymentTramsSelect, 
          Delivery_Term_ID : this.DeliveryMethodSelect,                					
