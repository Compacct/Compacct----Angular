...
@Component({
  selector: 'app-micl-finish-master-product',
  templateUrl: './micl-finish-master-product.component.html',
  styleUrls: ['./micl-finish-master-product.component.css'],
  providers: [MessageService],
  encapsulation: ViewEncapsulation.None
})
export class MiclFinishMasterProductComponent implements OnInit {
  tabIndexToView= 0;
  items:any = [];
  buttonname = "Create";
  menuList:any =[];
  FinishBrowseList:any = [];
  ProductTypeData:any =[];
  productSubTypeData:any=[];
  Objproduct: product =new product();
  ObjFinancialComponentData = new Financial();
  FinishProductFormSubmitted = false;
  isvisible =undefined
  Spinner = false;
  can_popup = false;
  act_popup = false;
  ...
  ngOnInit() {
    ...
    this.header.pushHeader({
      Header: "Finish Master Product",
      Link: " Production Management -> Finish Master Product"
    })
  }
  TabClick(e) {
  ...
  getBrowseFinishProduct(){
    const obj = {
      ...
      "Report_Name_String":"Browse_Finish_Master_Product"
    }
...
ViewProductSubType(){
  this.productSubTypeData = [];
  this.getProductSubTyp(this.Objproduct.Product_Type_ID);
  setTimeout(() => {
...
saveData(valid:any){
  ...
  this.FinishProductFormSubmitted = true;
  ...
  GetEditMasterProduct(uid){
    ...
    const obj = {
      "SP_String": "SP_Production_Finish_Master_Product",
      "Report_Name_String": "Get_Finish_Master_Product",
      "Json_Param_String": JSON.stringify([tempobj]) 
     }
     this.GlobalAPI.getData(obj).subscribe((data:any)=>{
