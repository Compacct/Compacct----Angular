...
ValidateGST() {
  var txtGSTCard:any = document.getElementById("txtGSTCard");
  var regex = /^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9]{1}Z[a-zA-Z0-9]{1}$/;
  if (regex.test(txtGSTCard.value.toUpperCase())) {
      return this.GSTvalidFlag = false;
  } else {
      return this.GSTvalidFlag = true;
  }
}
GetSearchedlist(){
  if (!this.Pan_No || this.PANvalidFlag) {
    this.compacctToast.clear();
    this.compacctToast.add({
      key: "compacct-toast",
      severity: "error",
      detail: "Invalid PAN NO"
    });
    // console.log("panno ===", this.Pan_No)
    return false;
  }
  if (!this.GST_No || this.GSTvalidFlag) {
    this.compacctToast.clear();
    this.compacctToast.add({
      key: "compacct-toast",
      severity: "error",
    ...
    // console.log("gstno ===", this.GST_No)
    return false;
  }
  
  console.log("panno ===", this.Pan_No.toUpperCase())
  console.log("gstno ===", this.GST_No.toUpperCase())
}
