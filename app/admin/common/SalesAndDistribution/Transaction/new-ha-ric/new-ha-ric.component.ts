...
@Component({  
  selector: 'app-new-ha-ric',
  templateUrl: './new-ha-ric.component.html',
  styleUrls: ['./new-ha-ric.component.css'],
  providers: [MessageService] ,
  encapsulation: ViewEncapsulation.None  
})

export class NewHaRicComponent implements OnInit {
  //items = [];
  cols = [];
  menuList = [];
  Spinner = false;
  ...
  ngOnInit() {
    ...
    this.serviceForm = this.fb.group({  
      ...
      NEW_HA_Colour: ['', Validators.required],
      NEW_HA_Acc: ['', Validators.required], 
      NEW_HA_CUS_For_Ear: ['LEFT'],
      Patient_Name: ['', Validators.required],
      NEW_HA_RIC_TYPE: ['', Validators.required],
      NEW_HA_RIC_Length: ['', Validators.required]      
    });
...
filter(){  
 ...
 this.dataNotFound = false;
  ...
  if (this.searchForm.invalid) {
    ...
    this.$http.get('/GHC_Inward_Outward_Register/NEW_HA_RIC_Browse?from_date=' + From_Txn_Date + '&to_date=' + To_Txn_Date + '&Txn_Type=NEW_HA_RIC&Input_Type=' + this.fval2.Search_Input_Type.value)
    .subscribe((data: any) => {
  ...
  onFormSubmit(){
    ...
    if (this.serviceForm.invalid) {
      ...
      const apiInput = {       
        ...
        "NEW_HA_Colour": this.fval.NEW_HA_Colour.value, 
        "NEW_HA_Acc": this.fval.NEW_HA_Acc.value,
        "NEW_HA_CUS_For_Ear": this.fval.NEW_HA_CUS_For_Ear.value,
        "Patient_Name": this.fval.Patient_Name.value,
        "NEW_HA_RIC_TYPE": this.fval.NEW_HA_RIC_TYPE.value,
        "NEW_HA_RIC_Length": this.fval.NEW_HA_RIC_Length.value
      };

      console.log('apiInput =', apiInput);
      const UrlAddress = '/GHC_Inward_Outward_Register/NEW_HA_RIC_CREATE_EDIT';
      const obj = { 
        'GHC_Inward_Outward_NEW_HA_RIC_String': JSON.stringify([apiInput])
      };    
...
edit(Inward_Outward_ID) {  
    ...
    this.$http.get('/GHC_Inward_Outward_Register/NEW_HA_RIC_Get_Data_For_Edit?Inward_Outward_ID=' + Inward_Outward_ID)
    .subscribe((data: any) => {
      ...
      this.serviceForm.get('NEW_HA_Colour').setValue(this.serviceData[0].NEW_HA_Colour);
      this.serviceForm.get('NEW_HA_Acc').setValue(this.serviceData[0].NEW_HA_Acc);  
      this.serviceForm.get('NEW_HA_CUS_For_Ear').setValue(this.serviceData[0].NEW_HA_CUS_For_Ear);  
      this.serviceForm.get('Patient_Name').setValue(this.serviceData[0].Patient_Name);  
      this.serviceForm.get('NEW_HA_RIC_TYPE').setValue(this.serviceData[0].NEW_HA_RIC_TYPE); 
      this.serviceForm.get('NEW_HA_RIC_Length').setValue(this.serviceData[0].NEW_HA_RIC_Length); 
      this.dateSelected();
    });
}
  ...
  clearData() {
    ...
    this.serviceForm.get('NEW_HA_CUS_For_Ear').setValue('LEFT');
  ...
  onConfirm() {  
      this.$http.post('/GHC_Inward_Outward_Register/NEW_HA_RIC_Delete?Inward_Outward_ID=' + this.deleteRowId, {})
      .subscribe((res: any) => {
