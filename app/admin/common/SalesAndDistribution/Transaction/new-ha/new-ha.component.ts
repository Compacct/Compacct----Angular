...
@Component({  
  selector: 'app-new-ha',
  templateUrl: './new-ha.component.html',
  styleUrls: ['./new-ha.component.css'],
  providers: [MessageService] ,
  encapsulation: ViewEncapsulation.None  
})

export class NewHAComponent implements OnInit {
  //items = [];
  cols = [];
  menuList = [];
  Spinner = false;
...
filter(){  
 ...
 this.dataNotFound = false;
  ...
  if (this.searchForm.invalid) {
    ...
    this.$http.get('/GHC_Inward_Outward_Register/NEW_HA_Browse?from_date=' + From_Txn_Date + '&to_date=' + To_Txn_Date + '&Txn_Type=NEW_HA&Input_Type=' + this.fval2.Search_Input_Type.value)
    .subscribe((data: any) => {
  ...
  onFormSubmit(){
    ...
    if (this.serviceForm.invalid) {
      ...
      const UrlAddress = '/GHC_Inward_Outward_Register/NEW_HA_CREATE_EDIT';
      const obj = { 
        'GHC_Inward_Outward_NEW_HA_String': JSON.stringify([apiInput])
      };    
...
edit(Inward_Outward_ID) {  
    ...
    this.$http.get('/GHC_Inward_Outward_Register/NEW_HA_Get_Data_For_Edit?Inward_Outward_ID=' + Inward_Outward_ID)
    .subscribe((data: any) => {
  ...
  onConfirm() {  
      this.$http.post('/GHC_Inward_Outward_Register/NEW_HA_Delete?Inward_Outward_ID=' + this.deleteRowId, {})
      .subscribe((res: any) => {
