...
import { EventSettingsModel ,  DayService, WeekService,
  TimelineViewsService, TimelineMonthService, View ,
  ScheduleComponent, CellClickEventArgs , EventRenderedArgs ,
  WorkWeekService, MonthService, AgendaService} from '@syncfusion/ej2-angular-schedule';
...
declare var $: any;
@Component({
  selector: 'app-compacct.syn',
  templateUrl: './compacct.syn.component.html',
  providers: [DayService, WeekService, WorkWeekService, MonthService,
     AgendaService , TimelineViewsService, TimelineMonthService , MessageService] ,
  styleUrls: ['./compacct.syn.component.css'] ,
  encapsulation: ViewEncapsulation.None ,
})
export class CompacctSynComponent implements OnInit {
  display = false;
  url = window['config'];
  ...
  SalesteamList = [];
  ReferenceDoctorList = [];
  optionsYesNo = [{ 'name': 'Yes', 'value': true }, { 'name': 'No', 'value': false }];
  StatusList = [
      { 'Name': 'H.A Accessories Sale', 'Consultancy_Type': 'Hearing' },
      { 'Name': 'For Information Only', 'Consultancy_Type': 'Hearing' },
      { 'Name': 'Appointment for purchase of Hearing Aid', 'Consultancy_Type': 'Hearing' },
      { 'Name': 'Appointment for purchase of Testings', 'Consultancy_Type': 'Hearing' },
      { 'Name': 'Patient Entry', 'Consultancy_Type': 'Speech' },
      { 'Name': 'Keep it in my followup', 'Consultancy_Type': 'Speech' }
  ];
  PreviousCompanyList = [
      { 'Name': 'ReSound' }, { 'Name': 'Interton' }, { 'Name': 'Siemens' },
      { 'Name': 'Signia' }, { 'Name': 'Starkey' }, { 'Name': 'Phonak' },
      { 'Name': 'Unitron' }, { 'Name': 'Widex' }, { 'Name': 'Oticon' },
      { 'Name': 'Beltone' }, { 'Name': 'Bernafon' }, { 'Name': 'Hansaton' },
      { 'Name': 'Other' }
  ];
  CustomerChoiceList = [{ 'Name': 'Low Budget' }, { 'Name': 'Mid Budget' }, { 'Name': 'High Budget' }];
  ChanceToBuyList = [{ 'Name': 'Within 5 days' }, { 'Name': 'Within 15 Days' }, { 'Name': 'Within 30 days' }];
  ...
  @ViewChild('menuObj', { static: false })
  ...
  '<div class="image" style="color:${CategoryColor}">' +
  '<i class="fa fa-fw ${className} fa-3x"></i></div>' +
  '<div class="content-area"><div class="name" >${Name}' +
  '<br><span class="badge"  style="background-color:${CategoryColor}; color:black; font-weight:600;">${status}</span> </></div>' +
  ...
  ngOnInit() {
    this.items = [ 'PATIENT DETAILS', 'APPOINTMENT DETAILS' , 'BILLING & PAYMENT & CHALLAN'];
    this.Header.pushHeader({'Header' : 'Appointment',
    'Link' : '   CRM -> Clinic -> Appointment'});
    this.eventSettings = { dataSource: <Object[]>extend([], this.AppoEvents, null, true),
  ...
  GetCostCenter() {
    ...
    this.commonApi.getCostCenter().subscribe((data: any) => {
      ...
      const defaultObj  =  $.grep(this.CostCenterList, function (value) {
        return value.Cost_Cen_ID === cookiesCostCenter; })[0];
  ...
  getAllConsultancy() {
    this.$http.get(this.url.apiGetAllDataMasterConsultancy).subscribe((data: any) => {
  ...
  getAllConsultancySub() {
    this.$http.get(this.url.apiGetAllDataConsultancySubTypeAppointment).subscribe((data: any) => {
  ...
  getPackageList(footFallID) {
    if (footFallID) {
      ...
      this.$http.get(this.url.apiGetPackageListAppointment , { params : obj} ).subscribe((data: any) => {
  ...
  getPackageSubType(footFallID , packageID) {
    if (footFallID && packageID) {
      ...
      this.$http.get(this.url.apiGetPackageSubTypeForAppointment , { params : obj} ).subscribe((data: any) => {
  ...
  getDoctorList() {
    ...
    if (this.objAppointment.Consultancy_Type && this.appointmentDate) {
        ...
        .set('Cost_Cen_ID', this.objAppointment.Cost_Cen_ID ? this.objAppointment.Cost_Cen_ID.toString() : '0')
        .set('Appo_Dt', this.dateApi.dateConvert(moment(this.appointmentDate, 'YYYY-MM-DD')['_d']));
          this.$http.get(this.url.apiGetDoctorsAppointmentHearing, {params : obj} ).subscribe((data: any) => {
...
ChangeSearchCostCenter (CostCenID) {
    ...
    this.costCenterName = $.grep(this.CostCenterList, function (value) {
      return value.Cost_Cen_ID === Number(CostCenID); })[0].Cost_Cen_Name;
    } else {
      this.costCenterName = '';
    }
      this.GetAppointmentList();
...
changeAppointmentDate (appoDate) {
    if (appoDate) {
      this.objAppointment.Appo_Dt = this.dateApi.dateConvert(moment(appoDate, 'YYYY-MM-DD')['_d']);
      }
}
...
changeDoctor(Schedule_ID) {
    if (Schedule_ID) {
        const obj = $.grep(this.doctorList, function (value) { return value.Schedule_ID === Number(Schedule_ID); })[0];
...
changeName (footFallID) {
  if ( footFallID) {
    footFallID ? this.getLastFollowupSetTo(footFallID) : this.objFollowupDetails.Sent_To = undefined;
...
changePackageSubType (ConsSubID) {
  this.tempConsultancyPackageSubtypeDuration = 0;
  if (ConsSubID) {
      const obj = $.grep(this.consultancySubList, function (value) { return value.Cons_Sub_ID === ConsSubID; })[0];
...
changeConsultancyType (consultancyType) {
 ...
 // this.allConsultancyList = [];
  ...
  this.SelectConsultancy = [];
  if (consultancyType) {
    this.allConsultancyList =  this.getConsultancyTypeWiseConsultancy(consultancyType);
    }
   this.getDoctorList();
  this.GetAppointmentList();
}
changeMobile(param) {
  param && param.length === 10 ? this.getLeadOrRegistrationPatientInfo(param) : this.patientNameList = [];
}
changeDoctorForUpdate (ScheduleID) {
  this.objUpdateDoctor.Doctor_ID = undefined;
  this.objUpdateDoctor.Schedule_ID = undefined;
  if (ScheduleID) {
      const obj = $.grep(this.doctorListForUpdate, function (value) { return value.Schedule_ID === Number(ScheduleID); })[0];
...
changeRescheduleAppointmentDoctor (ScheduleID) {
  ...
  if (ScheduleID) {
      const obj = $.grep(this.doctorListForReschedule, function (value) { return value.Schedule_ID === Number(ScheduleID); })[0];
...
GetDistrict (stateparm) {
  ...
  if (stateparm) {
    ...
    this.$http.get(this.url.apiGetDistrict, {params : obj} ).subscribe((data: any) => {
...
GetStateDistrict (PinParam) {
  ...
  if (PinParam && Number(PinParam.length) === 6) {
    ...
    this.$http.get(this.url.apiGetStateDistrict, {params : obj} ).subscribe((data: any) => {
      if (data.PIN) {
      this.Districts = data ? data : [];
      // this.Districts.push({})
...
getDoctorListForUpdate (ConsultancyType, CostCenID, appointmentDate) {
  const obj = new HttpParams()
      ...
      this.$http.get(this.url.apiGetDoctorsAppointment , { params : obj} ).subscribe((data: any) => {
...
getDoctorListForRescheduleAppointment () {
  if (this.ConsultancyTypeForReschedule && this.rescheduleAppointmentDate) {
      ...
      .set('Cost_Cen_ID',  this.objRescheduleAppointment.Cost_Cen_ID ? this.objRescheduleAppointment.Cost_Cen_ID.toString() : '0')
      .set('Appo_Dt', this.dateApi.dateConvert(this.rescheduleAppointmentDate));
      this.$http.get(this.url.apiGetDoctorsAppointment , { params : obj} ).subscribe((data: any) => {
...
getLastFollowupSetTo (footFallID) {
  const obj = new HttpParams()
  .set('Foot_Fall_Id', footFallID);
  this.$http.get(this.url.apiGetLastFollowup , { params : obj} ).subscribe((data: any) => {
    this.objFollowupDetails.Sent_To = data ? JSON.parse(data)[0].Sent_To : undefined;
  });
}
getLeadOrRegistrationPatientInfo(param) {
  const obj = new HttpParams()
  .set('Mobile', param);
  this.$http.get(this.url.apiGetRegistrationPatientInfoFromMobileNoHearing, {params : obj}).subscribe((data: any) => {
...
getRefDoctorList () {
  ...
  const obj = new HttpParams()
  .set('Enq_Source_ID',  this.PatientInfo['Enq_Source_ID']);
  this.$http.get(this.url.apiGetEnquerySourceSubMaster , { params : obj} ).subscribe((data: any) => {
...
checkAppointmentTime () {
  const doctorFromTime = Date.parse('01/01/2011' + ' ' + moment(this.doctorFromTime).format('HH:mm')  + ':00');
  const doctorToTime = Date.parse('01/01/2011' + ' ' +  moment(this.doctorToTime).format('HH:mm') + ':00');
  const appointmentTime = Date.parse('01/01/2011' + ' ' + moment(this.appointmentTime , 'HH:mm').format('HH:mm') + ':00');
  ...
  let tempDoctorLength = 0;
    if (this.objAppointment.Doctor_ID) {
      const id = this.objAppointment.Doctor_ID;
        tempDoctorLength = $.grep(this.AppointmentList,
          function (value) { return value.Doctor_ID === id; }).length;
    }
...
getConsultancyTypeWiseConsultancy (consultancyType) {
  if (consultancyType) {
      return $.grep(this.allConsultancyList, function (value) {
         return value.Consultancy_Type === consultancyType && value.Chargeable === true; });
  }
}
checkAppointmentTimeForReschedule () {
  const doctorFromTime = Date.parse('01/01/2011' + ' ' + moment(this.rescheduleDoctorFromTime).format('HH:mm') + ':00');
  const doctorToTime = Date.parse('01/01/2011' + ' ' +  moment(this.rescheduleDoctorToTime).format('HH:mm') + ':00');
  const appointmentTime = Date.parse('01/01/2011' + ' ' +  moment(this.rescheduleAppointmentTime , 'HH:mm').format('HH:mm') + ':00');
  if (appointmentTime >= doctorFromTime && appointmentTime <= doctorToTime) {
      return true;
  } else {
    this.compacctToast.clear();
...
GetAppointmentList () {
    ...
    .set('Schedule_ID', this.objSearchAppointment.Schedule_ID ? this.objSearchAppointment.Schedule_ID : '0')
    .set('Cost_Cen_ID', this.objSearchAppointment.Cost_Cen_ID ? this.objSearchAppointment.Cost_Cen_ID : '0')
    .set('Appo_Dt', this.dateApi.dateConvert(moment(this.appointmentDate, 'YYYY-MM-DD')['_d']))
    .set('Consultancy_Type', this.objSearchAppointment.Consultancy_Type ? this.objSearchAppointment.Consultancy_Type : '0')
    // this.scheduleObj.showSpinner();
 
    this.$http.get(this.url['apiGetDoctorScheduleAppointmentHearing'], { params }).subscribe((data: any) => {
...
intializeAppoEvents (data: []) {
    this.AppoEvents = [];
    this.AppointmentList = [];
    this.scheduleObj.eventSettings.dataSource =  <Object[]>extend([], this.AppoEvents, null, true);
    this.TotalAppointment = 0;
    this.TotalConsultancy = 0;
    this.TotalRescedule = 0;
    if (data.length) {
      for (let i = 0; i < data.length; i++) {
        this.AppoEvents.push({ id: data[i]['Appo_ID'],
                               allDay: false,
    ...
    this.TotalConsultancy = $.grep(this.AppoEvents, function(value) { return value.status === 'Consultancy Done'; }).length;
    this.TotalRescedule = $.grep(this.AppoEvents, function(value) { return value.status === 'Reschedule'; }).length;
    this.scheduleObj.eventSettings.dataSource =  <Object[]>extend([], this.AppoEvents, null, true);
    }
}
changeSchedule(e) {
...
fecthColorStatus(AppoObj) {
    if (AppoObj.Status) {
      if (AppoObj.Status === 'Appointment' && moment().isAfter(AppoObj.Appo_Dt)) {
        return '#c1c1c1';
    } else {
...
fetchIcons (Status) {
    ...
    if (Status === 'Appointment' ) { className = 'fa-user-md'}
    if (Status === 'Cancel' ) { className = 'fa-remove'; }
    if (Status === 'Reschedule' ) { className = 'fa-repeat'; }
    if (Status === 'Consultancy Done' ) { className = 'fa-stethoscope'; }
    if (Status === 'Consultancy Bill Done' ) { className = 'fa-money'; }
    if (Status === 'Package Booked' ) { className = 'fa-medkit'; }
    if (Status === 'Payment Done' ) { className = 'fa-money'; }
    if (Status === 'Therapy Booked' ) { className = 'fa-heartbeat'; }
    return className;
}
intializeEventsData (appoid: string) {
    ...
    if (appoid) {
     // this.$CompacctAPI.compacctSpinnerShow();
      ...
      this.EventDetailsObj = $.grep(this.AppointmentList, function(value) { return value.Appo_ID === id; })[0];
      console.log(this.EventDetailsObj);
...
GetDynamicallyAllData(footfallId) {
    ...
    if (footfallId) {
      ...
      const RegistrationDetails = this.$http.get(this.url['apiGetOneLeadHearing'], { params });
      params = new  HttpParams().set('Foot_Fall_ID', footfallId);
      const AppointmentOfPatient =  this.$http.get(this.url['apiGetAppointmentHearingFootFallIDWise'], { params });
      const billingListOfPatient =  this.$http.get(this.url['apiGetBillingDetailsFootFallIDWise'], { params });
      const paymentListOfPatient =  this.$http.get(this.url['apiGetPaymentDetailsFootFallIDWise'], { params });
      const paymentVoucherListOfPatient = this.$http.get(this.url['apiGetPaymentVoucherDetailsFootFallIDWise'], { params });
      const challanListOfPatient = this.$http.get(this.url['apiGetChallanDetailsFootFallIDWise'], { params });
      const AdvanceOrderPatient =  this.$http.get(this.url['apiGetAdvanceOrderDetailsFootFallIDWise'], { params });
      const saleReturnListOfPatient =  this.$http.get(this.url['apiGetSaleReturnDetailsFootFallIDWise'], { params });
      const productSaleListOfPatient =  this.$http.get(this.url['apiGetProductSaleDetailsFootFallIDWise'], { params });


      forkJoin([RegistrationDetails , AppointmentOfPatient , billingListOfPatient ,
         paymentListOfPatient , paymentVoucherListOfPatient , challanListOfPatient ,
          AdvanceOrderPatient , saleReturnListOfPatient , productSaleListOfPatient]).subscribe((result: any) => {
          console.log(result);
...
showAddAppoModal(data) {
    ...
    console.log(moment(data.startTime , 'YYYY-MM-DD')['_d']);
    // this.appointmentDate =  moment(data.startTime , 'YYYY-MM-DD').toDate();
    this.appointmentDate = moment(new Date(data.startTime)).format( 'YYYY-MM-DD');
...
async showRescheduleModal (obj) {
    ...
    if (obj) {
      const objParam = new HttpParams()
      .set('Appo_ID', obj.id);
      const t: any = await this.$http.get(this.url.apiCheckAppointmentStatus,
        { params : objParam  , responseType: 'text'} ).toPromise();
        this.checkAppointmentStatus = t;

        if ((obj.status === 'Consultancy Done' || obj.Status === 'Billed')
        ...
        if (this.checkAppointmentStatus === 'True') {
            ...
            this.rescheduleAppointmentDate = moment(new Date(obj.Appo_Dt)).format( 'YYYY-MM-DD');
            this.rescheduleAppointmentTime = moment(new Date(obj.Appo_Dt)).format( 'HH:mm');
           ...
           // this.rescheduleAppointmentTime = new Date(obj.Appo_Dt);
            ...
            this.tempDuration = obj.Cons_Duration ? obj.Cons_Duration : obj.Con_Sub_Duration;
    ...
    console.log(data);
      ...
      if (data) {
        ...
        const t: any = await this.$http.get(this.url.apiCheckAppointmentStatus,
          { params : objParam  , responseType: 'text'} ).toPromise();
          this.checkAppointmentStatus = t;
          if (this.checkAppointmentStatus === 'True' || data.status === 'Consultancy Done') {
              this.getDoctorListForUpdate(data.Consultancy_Type, data.Cost_Cen_ID, data.Appo_Dt);
              this.objUpdateDoctor.Appo_ID = data.id;
              this.objUpdateDoctorTime = new Date(data.Appo_Dt);
              this.tempDuration = data.Cons_Duration ? data.Cons_Duration : data.Con_Sub_Duration;
              this.objCheckAppointmentTime.Duration = this.tempDuration;
...
onContextMenuBeforeOpen(args: BeforeOpenCloseMenuEventArgs): void {
    ...
    if (this.selectedTarget.classList.contains('e-appointment')) {
     const  eventObj = <{ [key: string]: Object }>this.scheduleObj.getEventDetails(this.selectedTarget);
        console.log(eventObj);
        if (eventObj.status === 'Consultancy Done') {
            if (this.CompacctClientData.CompanyShortName !== 'AHSCC') {
                this.menuObj.hideItems(['AddAppo' , 'UpdateAppo', 'ConsultDone'], true);
                this.menuObj.showItems(['RescheduleAppo', 'UpdateDoctor', 'CanelAppo',
                  'PatientDetails', 'FeedbackAppo' ], true);
            } else {
            ...
            if (eventObj.Doctor_ID && this.CompacctClientData.CompanyShortName === 'MSHCPL') {
          ...
          this.menuObj.hideItems(['AddAppo' , 'FeedbackAppo' , 'ConsultDone'], true);
          this.menuObj.showItems(['RescheduleAppo', 'UpdateDoctor', 'CanelAppo' , 'UpdateAppo', 'PatientDetails'], true);
        }
        return;
    }
    this.menuObj.hideItems(['RescheduleAppo', 'UpdateDoctor', 'CanelAppo', 'ConsultDone', 'UpdateAppo',
     'PatientDetails', 'FeedbackAppo' ], true);
      ...
      if (valid) {
        ...
        if (this.objAppointment.Doctor_ID) {
          ...
          if (timeValidate && numberOfPatient) {
                ...
                'Appo_Dt': this.dateApi.dateConvert(moment(this.appointmentDate, 'YYYY-MM-DD')['_d'])
              ...
              this.$http.post(this.url.apiCheckAppointmentTime, ParamString).subscribe((data: any) => {
                    ...
                    const tempList = JSON.parse(data)[0];
                        if (tempList.Remarks === 'DONE') {
                          ...
                          console.log(this.ExistAppoList);
                        } else {
...
createFollowUp() {
      ...
      this.$http.post(this.url.apiCreateCRMFollowUp, this.objFollowupDetails)
          .subscribe((data: any) => {
...
saveAppointment (followupID) {
    ...
    this.objAppointment.Appo_Dt = this.dateApi.dateConvert(moment(this.appointmentDate, 'YYYY-MM-DD')['_d'])
    ...
    if (this.SelectConsultancy.length) { // adding consultancy wise multiple appointment 
      this.FetchAppoArrayList();
    } else {
        this.objAppointment.Appo_End_Dt = 
        this.convertAppoinmentEndTime(this.objAppointment.Appo_Dt, this.tempConsultancyPackageSubtypeDuration);
    }
    const AppoLength = this.AppoList.length;
    for (let j = 0; j < AppoLength; j++) { // post appointment list synchronously
       ...
       // var jsonString = { "Appointment_String": ctrl.convertJsonString(ctrl.AppoList[j]) };
        $.ajax({
            ...
            data: JSON.stringify({ 'Appointment_String': this.convertJsonString(this.AppoList[j]) }),
...
FetchAppoArrayList () {
    for (let i = 0; i < this.SelectConsultancy.length; i++) {
      ...
      const obj = $.grep(this.allConsultancyList, function (value) { return value.Cons_ID === id; })[0];

      this.objAppointment.Appo_End_Dt = this.convertAppoinmentEndTime(this.objAppointment.Appo_Dt, obj.Duration);
...
CreatePatient (valid) {
  ...
  if (valid) {
    ...
    this.objPatient.Posted_On = moment(new Date()).format('DD/MMM/YYYY HH:MM');
    const obj = {
      Enq_Clinic_String : this.convertJsonString(this.objPatient) ,
      ActionName: 'insert'
    };
    this.$http.post(this.url.apiCreateLeadHearing, obj).subscribe((data: any) => {
      if (data.success === true) {
...
UpdateDoctor (valid) {
  ...
  this.ExistAppoList = [];
  if (valid) {
      this.objUpdateDoctor.Appo_Dt = this.dateApi.dateConvert(moment(this.appointmentDate, 'YYYY-MM-DD')['_d']) +
       ' ' + moment(this.objUpdateDoctorTime , 'HH:mm').format('hh:mm A');
      this.objCheckAppointmentTime.Appo_Dt = this.objUpdateDoctor.Appo_Dt;
      this.objUpdateDoctor.Appo_End_Dt = this.convertAppoinmentEndTime(this.objUpdateDoctor.Appo_Dt, this.tempDuration);

     // this.existsPatientList = [];
      this.$http.post(this.url.apiCheckAppointmentTime, this.objCheckAppointmentTime).subscribe((data: any) => {
        const tempList = JSON.parse(data)[0];
        if (tempList.Remarks === 'DONE') {
...
updateNewDoctor () {
  const obj = { 'Update_Appointment_String': this.convertJsonString(this.objUpdateDoctor) };
  this.$http.post(this.url.apiUpdateDoctorForAppointment, obj).subscribe((data: any) => { 
    if (data.success === true) {
      this.GetAppointmentList();
      this.cleanUpdateDoctor();
...
checkRescheduleAppointment (valid) {
  ...
  this.ExistAppoList = [];
  if (valid) {
      this.objRescheduleAppointment.Appo_Dt = this.dateApi.dateConvert(this.rescheduleAppointmentDate) +
       ' ' +  moment(this.rescheduleAppointmentTime , 'HH:mm').format('hh:mm A');
      this.objRescheduleAppointment.Appo_End_Dt = this.convertAppoinmentEndTime(this.objRescheduleAppointment.Appo_Dt, this.tempDuration);
...
createRescheduleAppointment () {
  const obj = { 'Update_Appointment_String': this.convertJsonString(this.objRescheduleAppointment) };
  this.$http.post(this.url.apiRescheduleAppointmentHearing, this.objRescheduleAppointment).subscribe((data: any) => { 
    if (data.success === true) {
      this.GetAppointmentList();
      this.cleanRescheduleAppointment();
...
redirectHearingAidBill (obj) {
  console.log(this.CompacctClientData);
  if (obj) {
      window.open(this.CompacctClientData.Details[15].BillingType[0].HearingAidBill + '&recordid=' + window.btoa(obj.Foot_Fall_ID)
       + '&appoId=' + window.btoa(obj.id) + '&doctorID=' + window.btoa(obj.Doctor_ID));
  }
}
redirectAccessoriesBill(obj) {
  if (obj) {
      window.open(this.CompacctClientData.Details[15].BillingType[0].AccessoriesBill + '&recordid=' + window.btoa(obj.Foot_Fall_ID)
       + '&appoId=' + window.btoa(obj.id) + '&doctorID=' + window.btoa(obj.Doctor_ID));
  }
}
redirectServiceOrTestingBill (obj) {
  if (obj) {
      window.open(this.CompacctClientData.Details[15].BillingType[0].ServiceBill + '&recordid=' + window.btoa(obj.Foot_Fall_ID)
       + '&appoId=' + window.btoa(obj.id) + '&doctorID=' + window.btoa(obj.Doctor_ID));
  }
}
redirectBillCreation (obj) {
  if (obj) {
      window.open(this.CompacctClientData.Details[15].BillingType[0].BillCreation + '&recordid=' + window.btoa(obj.Foot_Fall_ID)
       + '&appoId=' + window.btoa(obj.id) + '&doctorID=' + window.btoa(obj.Doctor_ID));
  }
}
...
consultancyConfirmation  (AppoID) {
  if (AppoID) {
    this.$http.post(this.url.apiUpdateAppointmentStatus , { 'Appo_ID' : AppoID} ).subscribe((data: any) => {
         if (data.success === true) {
            this.GetAppointmentList();
            this.compacctToast.add({key: 'compacct-toast',
            severity: 'success',
...
cancelAppointment (AppoID) {
  if (AppoID) {
    this.$http.post(this.url.apiCancelAppointmentHearing , { 'Appo_ID' : AppoID} ).subscribe((data: any) => {
        if (data.success === true) {
            this.GetAppointmentList();
            this.compacctToast.add({key: 'compacct-toast',
            severity: 'success',
