...
export class TutoAccCashBankConfirmComponent implements OnInit {
  ...
  ngOnInit() {
    this.items = ["CASH CONFIRMATION", "BANK CONFIRMATION" , "CONFIRMED CASH" , "CONFIRMED BANK","URN CONFIRMATION"];
  ...
  GetUrnList () {
    const obj = {
      ...
      "Report_Name_String":"Get_URN_Not_Confirmed",
  ...
  URNConfirm(urnObj) {
    if(urnObj.URN_NO) {
      const tempObj = {
        URN_NO : urnObj.URN_NO,
        User_ID : this.$CompacctAPI.CompacctCookies.User_ID
      }
      const obj = {
        "SP_String":"Tutopia_Subscription_Loan_Disburse_SP",
        "Report_Name_String":"Update_URN_Confirmed",
        "Json_Param_String": "NA",
 ...
 //   this.SelectedDistOrderBy4 = [];
    ...
    this.BankConfirmedList.forEach((item) => {
      ...
      if (DistSource.indexOf(item.Order_Channel) === -1) {
        DistSource.push(item.Order_Channel);
        this.DistSource4.push({ label: item.Order_Channel, value: item.Order_Channel });
      }
    });
  }
  FilterDist4() {
    ...
    let DOrderBy = [];
    let DistSource = [];
    if (this.SelectedDistSource4.length) {
      DistSource = this.SelectedDistSource4;
    }
