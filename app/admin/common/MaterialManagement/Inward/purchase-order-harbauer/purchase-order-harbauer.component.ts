...
@Component({
  ...
  templateUrl: './purchase-order-harbauer.component.html',
  styleUrls: ['./purchase-order-harbauer.component.css'],
  providers: [MessageService] ,
  encapsulation: ViewEncapsulation.None
})
export class PurchaseOrderHarbauerComponent implements OnInit {
  tabIndexToView = 0;
  buttonname = 'Create';
  Spinner = false;
  seachSpinner = false;
...
GetDistinct() {
 ...
 if (DSubledgerName.indexOf(item.Vendor_Name) === -1) {
  DSubledgerName.push(item.Vendor_Name);
 this.DistSubledgerName.push({ label: item.Vendor_Name, value: item.Vendor_Name });
 }
...
if (this.SelectedDistSubledgerName.length) {
  this.SearchFields.push('Vendor_Name');
...
if (this.SearchFields.length) {
  let LeadArr = this.BackupSearchedlist.filter(function (e) {
    return (DSubledgerName.length ? DSubledgerName.includes(e['Vendor_Name']) : true)
...
FinalSave(col) {
  this.DocNoFinal = undefined;
  if(col.Doc_No){
   this.DocNoFinal = col.Doc_No
   this.compacctToast.clear();
   this.compacctToast.add({
     key: "c1",
     sticky: true,
      ...
      this.GlobalAPI.getData(obj).subscribe((data:any)=>{
        if (data[0].Column1){
          ...
          this.compacctToast.add({
            key: "compacct-toast",
            severity: "success",
            summary: "Final Save Id: " + this.DocNoFinal.toString(),
