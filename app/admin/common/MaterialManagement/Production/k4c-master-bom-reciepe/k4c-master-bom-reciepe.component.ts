...
export class K4cMasterBOMReciepeComponent implements OnInit {
  ...
  CheckDecimal(col) {
    //this.flag = false;
    var decimal = /^[-+]?[0-9]+\.[0-9]+$/;
    var inputtxt = col.Qty ? col.Qty.toString() : col.Qty;
    if (inputtxt && col.Decimal_Place && inputtxt.match(decimal)) {
      if (col.Decimal_Place === 1) {
        //  this.flag = false;
        const p1 = /^[-+]?[0-9]+\.[0-9]{1,1}$/;
        //console.log("col.Oty==", inputtxt.match(p1))
        return inputtxt.match(p1) ? false : true;
      }
      if (col.Decimal_Place === 2) {
        //this.flag = false;
        const p2 = /^[-+]?[0-9]+\.[0-9]{2,2}$/;
        //console.log("col.Oty==", inputtxt.match(p2))
        return inputtxt.match(p2) ? false : true;
      }
      if (col.Decimal_Place === 3) {
        //this.flag = false;
        const p3 = /^[-+]?[0-9]+\.[0-9]{3,3}$/;
       // console.log("col.Oty==", inputtxt.match(p3))
        return inputtxt.match(p3) ? false : true;
      }
    } else {
      if (inputtxt && col.Decimal_Place === 0) {
        //this.flag = false;
        const p0 = /^\d+$/;
       // console.log("col.Oty==", inputtxt.match(p0))
        return inputtxt.match(p0) ? false : true;
      } else {
        return !col.Qty ? false : true;
  ...
  ValidQtyCheck() {
    let ValidFlag = false;
    for (let index = 0; index < this.getrawmaterialtabledata.length; index++) {
      const element = this.getrawmaterialtabledata[index];
      if (this.CheckDecimal(element)) {
        ValidFlag = false;
        this.compacctToast.clear();
        this.compacctToast.add({
          key: "compacct-toast",
          severity: "warn",
          summary: "Validation",
          detail: "Invalid Qty in Raw Material."
        });
        return ValidFlag;
      } else {
        ValidFlag = true;
    ...
    for (let index = 0; index < this.getsemifinishedtabledata.length; index++) {
      const element = this.getsemifinishedtabledata[index];
      if (this.CheckDecimal(element)) {
        ValidFlag = false;
        this.compacctToast.clear();
        this.compacctToast.add({
          ...
          severity: "warn",
          summary: "Validation",
          detail: "Invalid Qty in Semi Finished Material."
        });
        return ValidFlag;
      } else {
        ValidFlag = true;
