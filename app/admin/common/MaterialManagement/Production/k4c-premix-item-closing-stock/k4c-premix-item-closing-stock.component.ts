...
@Component({
  selector: 'app-k4c-premix-item-closing-stock',
  templateUrl: './k4c-premix-item-closing-stock.component.html',
  styleUrls: ['./k4c-premix-item-closing-stock.component.css'],
  providers: [MessageService],
  encapsulation: ViewEncapsulation.None
})
export class K4cPremixItemClosingStockComponent implements OnInit {
  items:any = [];
  Spinner = false;
  seachSpinner = false
  ShowSpinner = false;
  ...
  // FOR PRODUCT TABLE
  GetProduct(valid){
    this.PremixInvCSSubmitted = true;
    if(valid){
  ...
  this.GlobalAPI.getData(obj).subscribe((data:any)=>{
   ...
   tempData.forEach(element => {
    var idealclosing = Number((element.Opening_Qty + element.Production_Qty) - element.Issue_Qty).toFixed(2);
    // element['Issue_Qty'] = undefined;
    element['Wastage_Qty'] = undefined;
    element['Remarks'] = undefined;
    element['Ideal_Closing_Qty'] = Number(idealclosing).toFixed(2);
    element['Closing_Qty'] = this.MaterialType_Flag === "Semi Finished" ?  Number(idealclosing).toFixed(2) : element.Closing_Qty;
    element['Wastage_Qty'] = 0;
    element['Variance'] = Number((element.Ideal_Closing_Qty - element.Closing_Qty) - element.Wastage_Qty).toFixed(2);
    if (this.MaterialType_Flag === "Semi Finished") {
    element['Receive_Qty'] = element.Production_Qty;
    }
    if(element['Variance'] > 0 || element['Wastage_Qty'] > 0){
      element['remarkdisabled'] = false;
    } else {
      element['remarkdisabled'] = true;
    }
    element['Consumption_Qty'] = Number(Number(Number(element.Opening_Qty + element.Receive_Qty) - element.Closing_Qty) - element.Wastage_Qty).toFixed(2);
 });
   this.ProductList = tempData;
   this.BackupProList = data;
   this.ShowSpinner = false;
   this.BackupIndentList = tempData;
    this.PremixInvCSSubmitted = false;
    this.GetProductType();
  ...
  console.log("Change Remarks")
   this.ProductList.forEach(el=>{
     if(obj.Product_ID === el.Product_ID){
      ...
      if(obj.Wastage_Qty > 0 || obj.Variance > 0){
        obj.remarkdisabled = false;
        var openrec = (obj.Opening_Qty + obj.Receive_Qty).toFixed(2);
        var subclosing = (Number(openrec) - obj.Closing_Qty).toFixed(2);
...
calculateIdealClosingQty(indx,col){
  this.ProductList[indx]['Ideal_Closing_Qty'] = 0;
  if(this.ProductList[indx]['Receive_Qty']) {
    var addopenpro = (this.ProductList[indx]['Opening_Qty'] + Number((this.ProductList[indx]['Receive_Qty']))).toFixed(2); 
    var subissue = (Number(addopenpro) - this.ProductList[indx]['Issue_Qty']).toFixed(2);
    this.ProductList[indx]['Ideal_Closing_Qty'] = Number(subissue).toFixed(2);
    this.calculatevariation(indx);
    this.ConsumptionCal(indx);
  }
  this.checkdecimal(col);
}
calculatevariation(indx){
  this.ProductList[indx]['Variance'] = 0;
  if(this.ProductList[indx]['Ideal_Closing_Qty'] || this.ProductList[indx]['Closing_Qty']) {
    var variance = Number((this.ProductList[indx]['Ideal_Closing_Qty'] - this.ProductList[indx]['Closing_Qty']) - this.ProductList[indx]['Wastage_Qty']).toFixed(2); 
    this.ProductList[indx]['Variance'] = Number(variance).toFixed(2);
    this.ConsumptionCal(indx);
  }
}
ConsumptionCal(indx){
  this.ProductList[indx]['Consumption_Qty'] = 0;
  if(this.ProductList[indx]['Receive_Qty'] || this.ProductList[indx]['Closing_Qty']){
    ...
    var consumptionqty = Number(subclosing);
    this.ProductList[indx]['Consumption_Qty'] = Number(consumptionqty).toFixed(2)
  ...
  if(this.ProductList[indx]['Wastage_Qty']){
    var openrec = (this.ProductList[indx]['Opening_Qty'] + Number((this.ProductList[indx]['Receive_Qty']))).toFixed(2);
    var subclosing = (Number(openrec) - this.ProductList[indx]['Closing_Qty']).toFixed(2);
    var consumptionqty = Number(Number(subclosing) - this.ProductList[indx]['Wastage_Qty']);
    this.ProductList[indx]['Consumption_Qty'] = Number(consumptionqty).toFixed(2)
  }
  //this.changeRemarks(indx);
  this.ProductList.forEach(el=>{
    if(this.ProductList[indx]['Product_ID'] === el.Product_ID){
     if(this.ProductList[indx]['Variance'] > 0 || this.ProductList[indx]['Wastage_Qty'] > 0){
      ...
      this.ObjProClosingStock.Brand_ID = data[0].Brand_ID ? data[0].Brand_ID : undefined;
       ...
       this.ObjProClosingStock.godown_id = data[0].godown_id;
         data.forEach(element => {
          var idealclosing = Number((element.Opening_Qty + element.Production_Qty) - element.Issue_Qty).toFixed(2);
          var variance = Number((Number(idealclosing) - element.Closing_Qty) - element.Wastage_Qty).toFixed(2)
           const  productObj = {
            Brand : element.Brand,
            Product_Type : element.Product_Type,
            Product_ID : element.Product_ID,
            ...
            Ideal_Closing_Qty : Number(idealclosing).toFixed(2),
            Closing_Qty : element.Closing_Qty,
            Variance : Number(variance).toFixed(2),
            Wastage_Qty : element.Wastage_Qty,
            Remarks :  element.Remarks,
            Consumption_Qty : element.Consumption_Qty,
            remarkdisabled : Number(variance) > 0 || element.Wastage_Qty > 0 ? false : true
          };
  ...
  this.ProductList = []
   if(this.QtyfilterFLag){
    this.BackupProList.forEach(el=>{
      ...
      if (this.MaterialType_Flag === "Raw Material") {
        if(Number(el.Closing_Qty) > 0){
          this.ProductList.push(el);
        }
      }
